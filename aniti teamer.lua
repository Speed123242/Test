...
game.Players:Chat(("HOI mr.anti-teamer knife"))
		wait(0.5)
                k:Stop()
    end
end)
...
game.Players:Chat(("Ya is sans bone :3"))
...
game.Players:Chat(("Regret for teaming =3"))
		Anim.AnimationId = "rbxassetid://"
		local k = game.Players.LocalPlayer.Character.Humanoid:LoadAnimation(Anim)
		k:Play()
		k:AdjustSpeed(1)
...
mouse.KeyDown:Connect(function(k) 
	if k == "x" then 
		...
		k:AdjustSpeed(2)
		for i,v in pairs(getconnections(game:GetService("ReplicatedStorage").Remotes.Effects.OnClientEvent)) do
        v:Fire({"Model", "Wind", game.Players.LocalPlayer.Backpack.Main.LockOnScript.LockOn.Value.HumanoidRootPart.CFrame})
        end
		local args = {
			[1] = getrenv()._G.Pass,
			[2] = game:service("Players").LocalPlayer.Backpack.Main.LockOnScript.LockOn.Value,
...
game.Players:Chat(("Weeeeeeeeeeee >:3"))
		local Anim = Instance.new("Animation")
